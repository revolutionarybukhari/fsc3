      ...
      <div className="inline-flex rounded-lg border border-border-subtle bg-surface p-1 gap-0.5">
        {tabs.map((tab) => (
          <button
            ...
            className={`relative px-4 py-2 rounded-md text-[12px] font-medium transition-colors ${
          ...
          >
            {tab}
            {activeSettingsTab === tab && (
              <span className="absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] w-4/5 rounded-full bg-accent" />
            )}
          </button>
        ))}
      </div>
      ...
      {activeSettingsTab === "Data" && (
        <div className="space-y-5 mt-6">
          ...
          <div className={cardCls}>
            ...
            <button
              type="button"
              onClick={() => addToast("File browser opened \u2014 select CSV to upload", "info")}
              className="w-full flex flex-col items-center justify-center gap-4 sm:gap-5 rounded-lg border-2 border-dashed border-border-subtle p-8 sm:p-12 cursor-pointer hover:border-accent hover:bg-white/[0.02] transition-colors"
            >
            ...
            <div className="space-y-4 sm:space-y-5">
              {connectors.map((c) => (
                <button
                  ...
                  className="w-full flex items-center justify-between rounded-lg border border-border-subtle bg-white/[0.02] py-4 px-4 text-left cursor-pointer hover:bg-white/[0.02] hover:bg-white/[0.04] transition-all"
                >
